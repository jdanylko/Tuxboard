...
public class DashboardService : IDashboardService
{
    ...
    public Dashboard CreateDashboardFrom(DashboardDefault template) => CreateFromTemplate(template);
    ...
    public async Task<List<WidgetPlacement>> GetWidgetsForTabAsync(DashboardTab tab) =>
        await _context.GetWidgetsForTabAsync(tab);
    ...
    public async Task<List<Widget>> GetWidgetsForAsync(int planId = 0) =>
        planId > 0
            ? await _context.Widget.Where(u => u.WidgetPlans.Any(i => i.PlanId == planId)).ToListAsync()

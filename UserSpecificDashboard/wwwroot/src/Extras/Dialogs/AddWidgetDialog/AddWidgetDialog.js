...
class AddWidgetDialog extends BaseDialog_1.BaseDialog {
    ...
    getDataId(elem) {
        return elem.getAttribute(common_1.dataId);
    }
    getWidgetDialog() {
    ...
    getSelectedSelector() {
        return common_1.noPeriod(this.widgetSelectionSelector);
    }
    ...
    hide() {
        const modal = bootstrap_1.Modal.getInstance(this.getWidgetDialog());
        if (modal) {
            modal.hide();
        }
    }
    ...
    addWidgetClick(ev) {
        ...
        const widgetId = widget.getAttribute(common_1.dataId);
    ...
    setupWidgetTabs() {
        ...
        tabTriggers.forEach((triggerEl) => {
            const tabTrigger = new bootstrap_1.Tab(triggerEl);
